...
export default function MarriagePlanningCalculator() {
    ...
    const calculateMarriagePlan = async () => {
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_DATA_API}/api/calculators/marriage-calculator?currentAge=${currentAge}&MarriageStartAge=${MarriageStartAge}&totalInvestment=${totalInvestment}&expectedReturn=${expectedReturn}&inflationRate=${inflationRate}&apikey=${process.env.NEXT_PUBLIC_API_KEY}`);
            if (res.status === 200) {
                const data = res.data
    ...
    return (
        <div className="lg:px-40 md:px-20 px-10 py-10">
