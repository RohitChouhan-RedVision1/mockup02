...
const FinancialHealth = () => {
    ...
    const fetchQuestions = async () => {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_DATA_API}/api/open-apis/health-questions?apikey=${process.env.NEXT_PUBLIC_API_KEY}`);
        ...
        <div className="max-w-3xl mx-auto my-32 p-6 bg-white shadow-lg rounded-lg">
            ...
            {!isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="p-3 rounded-lg shadow-lg w-[30rem] bg-white mt-10">
                        <InquiryForm onClose={() => {
                            setIsModalOpen(false);
                    ...
                    <div className="mb-4 ">
                        ...
                        <div className="flex items-center mb-2">
                            <input
                                ...
                                className="mr-2"
                            />
                            <label htmlFor={'no'} className="text-lg text-gray-800">No</label>
                        </div>
                    </div>
                    <Button
                        onClick={() => handleNextClick(questions[currentQuestionIndex]?.question)}
                        className="bg-[var(--rv-bg-secondary)] text-white px-4 py-2 rounded-lg hover:bg-[var(--rv-bg-primary)]"
                    >
                        Next
                    </Button>
                </div>
