...
const Services = ({servicedata}) => {
          ...
          <Slider {...settings}>
            {servicedata.map((service, index) => (
              <div key={index} className="px-4 ">
                 ...
                 <span className="group h-24 w-24 text-center border-2 border-[var(--secondary)] text-[var(--secondary)] rounded-full inline-flex items-center justify-center text-[50px] transition-all duration-500 hover:bg-[var(--secondary)] hover:text-white">
  <Image src={`/${service.imageSrc}`} alt={service.name} width={50} height={50} className="transition-all duration-500 group-hover:brightness-999" />
                  ...
                  <Button className="mt-4 h-[50px] border border-[var(--secondary)] inline-flex items-center gap-2 px-6 rounded-[30px] text-[var(--secondary)] text-[15px] font-medium uppercase transition-all duration-300 hover:bg-[var(--secondary)] hover:text-white cursor-pointer">
  <FaPlus /> Read More
</Button>
