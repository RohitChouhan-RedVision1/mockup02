...
//   <Button
//     onClick={onClick}
//     className={`hidden md:block md:absolute ${direction === 'prev' ? 'left-[-28px]' : 'right-[-28px]'} top-1/2 transform -translate-y-1/2 text-white bg-[#0E314D] p-3 rounded-4xl z-10`}
//   >
//     {direction === "prev" ? "\u2B9C" : "\u2B9E"}
//   </Button>
// );
...
//           >
//             <div
//               className={`h-64 rounded-lg shadow-lg p-8 text-center transition-all border border-[#C59F4A] duration-300 ease-in-out ${hoveredIndex === index ? 'bg-white text-black' : 'bg-[#C59F4A] text-white'}`}
//             >
//                 {hoveredIndex === index ? (
    ...
    <div className="py-4 md:py-12 bg-white px-2 md:px-0">
      ...
      <Slider {...settings}>
        {data.map((item, index) => (
          ...
          >
            <div
              className={`h-64 rounded-lg shadow-lg p-4 text-center transition-all border border-[#C59F4A] duration-300 ease-in-out bg-[#C59F4A] text-white `}
            >
